...
function loadDashboard(user) {
    ...
    const myRaffles = raffles.filter(r => r.creatorId === user.uid);

    // Participating (have tickets in)
    const participating = raffles.filter(r => {
        if (r.creatorId === user.uid) return false;
        return r.tickets && r.tickets.some(t => t.buyerId === user.uid);
    });
    ...
    raffles.forEach(r => {
        if (r.tickets) {
            totalTickets += r.tickets.filter(t => t.buyerId === user.uid).length;
        }
    });

    // Won raffles
    const wonRaffles = raffles.filter(r => r.winnerId === user.uid);
    ...
    container.innerHTML = raffles.map(raffle => {
      ...
      <div class="card" onclick="window.location.href='raffle.html?id=${raffle.id}'" style="cursor:pointer;">
        <div class="card-header">
          <h3 class="card-title">${escapeHtml(raffle.name)}</h3>
          ${statusBadge}
        </div>
